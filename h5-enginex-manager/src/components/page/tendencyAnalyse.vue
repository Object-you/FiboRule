	...
	<div class="tendencyAna">
		<div class="tendencyAna_header">
			<div>
				...
				<el-select v-model="engineId" :loading="enginelist.length == 0" style="margin-right: 20px;" filterable
					placeholder="请选择引擎" @change="getEngVersion">
					<el-option v-for="item in enginelist" :key="item.id" :label="item.name" :value="item.id">
					</el-option>
				</el-select>
				<el-select v-model="versionId" :loading="versionList.length == 0" style="margin-right: 20px;" filterable
					placeholder="请选择版本" @change="getData">
					<el-option v-for="item in versionList" :key="item.id" :label="item.label" :value="item.id">
					</el-option>
				</el-select>
				<el-date-picker v-model="time" type="daterange" align="right" unlink-panels :clearable="false"
		...
		<template v-if="versionId">
							...
							<div>
								...
								<div class="tendencyAna_summaryCont">
									...
									<div>
										...
										<div>
											<p
												:style="{ color: engineSummary.engine_call.growthRate > 1 ? '#0f0' : '#f00' }">
												{{ Math.abs(engineSummary.engine_call.growthRate * 100 - 100) }}%
												<i
													:class="engineSummary.engine_call.growthRate > 1 ? 'el-icon-top' : 'el-icon-bottom'"></i>
											</p>
							...
							<div>
								...
								<div class="tendencyAna_summaryCont">
									...
									<div>
										...
										<div>
											<p
												:style="{ color: engineSummary.decision_result.growthRate > 1 ? '#0f0' : '#f00' }">
												{{ Math.abs(engineSummary.decision_result.growthRate * 100 - 100) }}%
												<i
													:class="engineSummary.decision_result.growthRate > 1 ? 'el-icon-top' : 'el-icon-bottom'"></i>
											</p>
							...
							<div>
								...
								<div class="tendencyAna_summaryCont">
									...
									<div>
										...
										<div>
											<p
												:style="{ color: engineSummary.rule_hit.growthRate > 1 ? '#0f0' : '#f00' }">
												{{ Math.abs(engineSummary.rule_hit.growthRate * 100 - 100) }}%
												<i
													:class="engineSummary.rule_hit.growthRate > 1 ? 'el-icon-top' : 'el-icon-bottom'"></i>
											</p>
							...
							<div>
								...
								<div class="tendencyAna_summaryCont">
									...
									<div>
										...
										<div>
											<p
												:style="{ color: engineSummary.node_hit.growthRate > 1 ? '#0f0' : '#f00' }">
												{{ Math.abs(engineSummary.node_hit.growthRate * 100 - 100) }}%
												<i
													:class="engineSummary.node_hit.growthRate > 1 ? 'el-icon-top' : 'el-icon-bottom'"></i>
											</p>
							...
							<div>
								...
								<div class="tendencyAna_summaryCont">
									...
									<div>
										...
										<div>
											<p
												:style="{ color: engineSummary.scorecard.growthRate > 1 ? '#0f0' : '#f00' }">
												{{ Math.abs(engineSummary.scorecard.growthRate * 100 - 100) }}%
												<i
													:class="engineSummary.scorecard.growthRate > 1 ? 'el-icon-top' : 'el-icon-bottom'"></i>
											</p>
							...
							<div>
								...
								<div class="tendencyAna_summaryCont">
									...
									<div>
										...
										<div>
											<p
												:style="{ color: engineSummary.decision_tables.growthRate > 1 ? '#0f0' : '#f00' }">
												{{ Math.abs(engineSummary.decision_tables.growthRate * 100 - 100) }}%
												<i
													:class="engineSummary.decision_tables.growthRate > 1 ? 'el-icon-top' : 'el-icon-bottom'"></i>
											</p>
			...
			<el-row :gutter="20" style="margin-top: 20px;">
				...
				<el-col :span="12">
					<el-card shadow="hover" style="position: relative;">
						<el-table :data="option1TableData" style="width: 100%;height: 300px;" max-height="300"
							:default-sort="{ prop: 'resultCount', order: 'descending' }">
			...
			<el-row :gutter="20" style="margin-top: 20px;">
				<el-col :span="12">
					<el-card shadow="hover">
						<el-select v-model="option2Put" size="mini" multiple collapse-tags value-key="id"
							:loading="option2PutList.length == 0" style="margin-left: 20px;position: absolute;"
							filterable placeholder="请选择决策结果" :key="num">
							<el-option v-for="item in option2PutList" :key="item[0].id" :label="item[0].result"
								:value="item">
							</el-option>
						</el-select>
				...
				<el-col :span="12">
					<el-card shadow="hover" style="position: relative;">
						<el-table :data="option2TableData" style="width: 100%;height: 300px;" max-height="300"
							:default-sort="{ prop: 'resultCount', order: 'descending' }">
			...
			<el-row :gutter="20" style="margin-top: 20px;">
				<el-col :span="12">
					<el-card shadow="hover" style="position: relative;">
						<el-select v-model="option3Put" size="mini" multiple collapse-tags value-key="id"
							:loading="option3PutList.length == 0" style="margin-left: 20px;position: absolute;"
							filterable placeholder="请选择规则及版本" :key="num">
							<el-option v-for="item in option3PutList" :key="item[0].id"
								:label="item[0].ruleName + ':' + item[0].ruleVersionCode" :value="item">
							</el-option>
						</el-select>
				...
				<el-col :span="12">
					<el-card shadow="hover">
						<el-table :data="option3TableData" style="width: 100%;height: 300px;" max-height="300"
							:default-sort="{ prop: 'resultCount', order: 'descending' }">
			...
			<el-row :gutter="20" style="margin-top: 20px;">
				<el-col :span="12">
					<el-card shadow="hover" style="position: relative;">
						<el-select v-model="option4Put" size="mini" multiple collapse-tags value-key="id"
							:loading="option4PutList.length == 0" style="margin-left: 20px;position: absolute;"
							filterable placeholder="请选择节点" :key="num">
							<el-option v-for="item in option4PutList" :key="item[0].id" :label="item[0].nodeName"
								:value="item">
							</el-option>
						</el-select>
				...
				<el-col :span="12">
					<el-card shadow="hover">
						<!-- {{option4TableData.length}} -->
						<el-table :data="option4TableData" style="width: 100%;height: 300px;" max-height="300"
							:default-sort="{ prop: 'resultCount', order: 'descending' }">
							<el-table-column prop="result" label="命中" width="180">
							</el-table-column>
							<el-table-column prop="resultCount" label="总计值" sortable>
			...
			<el-row :gutter="20" style="margin-top: 20px;">
				<el-col :span="12">
					<el-card shadow="hover" style="position: relative;">
						<el-select v-model="option5Put" size="mini" :loading="option5PutList.length == 0"
							style="margin-left: 20px;position: absolute;z-index: 5;" filterable value-key="[0][0].id"
							placeholder="请选择评分卡节点" :key="num">
							<el-option v-for="item in option5PutList" :key="item[0][0].id"
								:label="item[0][0].scorecardName + ':' + item[0][0].scorecardVersionCode" :value="item">
							</el-option>
						</el-select>
				...
				<el-col :span="12">
					<el-card shadow="hover" style="position: relative;">
						<el-table :data="option5TableData" style="width: 100%;height: 300px;" max-height="300"
							:default-sort="{ prop: 'resultCount', order: 'descending' }">
							<el-table-column prop="result" label="命中" width="180">
							</el-table-column>
							<el-table-column prop="resultCount" label="总计值" sortable>
			...
			<el-row :gutter="20" style="margin-top: 20px;">
				<el-col :span="12">
					<el-card shadow="hover" style="position: relative;">
						<el-select v-model="option6Put" size="mini" :loading="option6PutList.length == 0"
							style="margin-left: 20px;position: absolute;z-index: 5;" filterable value-key="[0][0].id"
							placeholder="请选择入参" :key="num">
							<el-option v-for="item in option6PutList" :key="item[0][0].id"
								:label="item[0][0].decisonTablesName + ':' + item[0][0].decisonTablesVersionCode"
								:value="item">
							</el-option>
						</el-select>
				...
				<el-col :span="12">
					<el-card shadow="hover" style="position: relative;">
						...
						<el-table :data="option6TableData" style="width: 100%;height: 300px;" max-height="300"
							:default-sort="{ prop: 'resultCount', order: 'descending' }">
							<el-table-column prop="result" label="命中" width="180">
							</el-table-column>
							<el-table-column prop="resultCount" label="总计值" sortable>
	...
	watch: {
		option2Put: {
			handler: function () {
				...
				this.option2TableData = this.option2Put.map(value => {
					let num = 0
					value.forEach(item => {
						num += item.resultCount
					})
		...
		option3Put: {
			handler: function () {
				...
				this.option3TableData = this.option3Put.map(value => {
					let num = 0
					value.forEach(item => {
						num += item.hitCount
					})
					return {
						resultCount: num,
						result: value[0].ruleName + ':' + value[0].ruleVersionCode,
					}
				})
			},
			deep: true
		...
		option4Put: {
			handler: function () {
				...
				this.option4TableData = this.option4Put.map(value => {
					let num = 0
					value.forEach(item => {
						num += item.passCount
					})
				...
				console.log(this.option4Put, this.option4TableData)
			},
			deep: true
		},
		option5Put: {
