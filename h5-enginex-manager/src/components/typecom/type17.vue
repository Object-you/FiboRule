...
<template>
	<div class="type4 type" v-loading="loading">
		...
		<div style="border-top: 1px solid #ddd;padding: 5px ;">
			<p style="margin-top: 10px; margin-bottom: 10px;font-size: 14px;font-weight: bold;">当前选择的决策树：</p>
			<div style="margin-bottom: 10px;">
				<p v-if="checked!=null" style="color: #409EFF;">{{checked.name}}</p>
				<p v-if="checked==null" style="color: #999;">暂未选择决策树</p>
			</div>
			...
			<div style="margin-bottom: 10px;border-top: 1px solid #ddd;border-bottom: 1px solid #ddd;padding: 10px 0;">
				<el-button type="primary" round size="mini"
					@click="dialogTableVisible=true;radio=checked?checked.id:'';tempVersion = version">选择决策树</el-button>
			</div>
		</div>

		<endRule :data="terminationInfo" :list="[checked]" :onlyOne="true" @reSetTerminationInfo="reSetTerminationInfo"
			:valueType="2">
			<el-option label="决策树结果" :value="'17_'+data.id+'_terminal_result'"></el-option>
		...
		<el-dialog title="选择决策树" width="30%" :visible.sync="dialogTableVisible" :modal="true" :append-to-body="true">
	...
	export default {
		...
		methods: {
			crea() {
				...
				// }).then(res => {
					...
					if (this.data.nodeJson) {
						...
						this.radio = dataJson.decisionTreeList[0].decisionTreeId
						this.terminationInfo = dataJson.terminationInfo
						this.list.forEach(value => {
							if (value.id == this.radio) {
								this.checked = value
								this.versionList = value.versionList
								this.version = dataJson.decisionTreeList[0].versionId
							}
						})
					}
			...
			radioChange() {
				this.list.forEach(value => {
					if (value.id == this.radio) {
						...
						this.tempVersion = value.versionList[0].id
					}
				})
			},
			radioSure(e, type = 1) {
			...
			submit() {
				...
				let subObj = {
					"decisionTreeList": [{
						"decisionTreeId": this.checked.id,
			...
			async getType(param) {
				const data = await getDecisionTreeList(param);
				return data
			},
			async setType(param) {
				this.loading = true
		...
		computed: {
			list(){
				return this.$store.state.decisionTree
			},
			describe() {
				let str = ''
				if (this.list) {
					this.list.forEach(value => {
						value.versionList.forEach(item => {
